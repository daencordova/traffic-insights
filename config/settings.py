...
class CameraConfig(BaseModel):
    ...
    capture_buffer_size: int = Field(
        ...
        ge=1,
        le=10,
        description="Tamaño del buffer de captura de OpenCV (CV_CAP_PROP_BUFFERSIZE)",
    )
...
class TrackerConfig(BaseModel):
    ...
    enable_mht: bool = Field(default=False, description="Habilitar sistema MHT")
    ...
    enable_sensor_fusion: bool = Field(default=False, description="Habilitar fusión de sensores")
    ...
    fusion_min_observations: int = Field(
        2, ge=1, le=10, description="Mínimo de observaciones para fusionar"
    )
    fusion_max_history: int = Field(50, ge=10, le=200, description="Máximo historial de fusiones")
    fusion_particle_count: int = Field(
        500, ge=100, le=1000, description="Número de partículas (particle filter)"
    )

    fusion_visual_weight: float = Field(0.7, ge=0.0, le=1.0, description="Peso del sensor visual")
    fusion_depth_weight: float = Field(
        0.5, ge=0.0, le=1.0, description="Peso del sensor de profundidad"
    )
    fusion_thermal_weight: float = Field(0.4, ge=0.0, le=1.0, description="Peso del sensor térmico")
    fusion_motion_weight: float = Field(
        0.3, ge=0.0, le=1.0, description="Peso del sensor de movimiento"
    )

    enable_path_prediction: bool = Field(
        default=True, description="Habilitar predicción de trayectoria"
    )
    prediction_history_length: int = Field(
        30, ge=5, le=100, description="Longitud del historial para predicción"
    )
    prediction_horizon: float = Field(
        2.0, ge=0.5, le=10.0, description="Horizonte de predicción en segundos"
    )
    prediction_steps: int = Field(20, ge=5, le=50, description="Número de pasos de predicción")
    prediction_min_samples: int = Field(
        5, ge=2, le=20, description="Mínimo de muestras para predicción"
    )
    ...
    prediction_uncertainty_threshold: float = Field(
        0.7, ge=0.0, le=1.0, description="Umbral de incertidumbre"
    )
    ...
    tree_update_interval: float = Field(
        0.5, ge=0.1, le=2.0, description="Intervalo de actualización del KD-Tree en segundos."
    )
    ...
    def validate_mht_depth(cls, v: int, info) -> int:
        ...
        if enable_mht and v < MIN_MHT_DEPTH:
            raise ValueError(
                f"mht_max_depth debe ser al menos {MIN_MHT_DEPTH} si MHT está activo: {v}"
            )
        return v
    ...
    def validate_particle_count(cls, v: int, info) -> int:
        ...
        if enable_sensor_fusion and fusion_method == "particle_filter" and v < MIN_PARTICLE_COUNT:
            raise ValueError(
                f"fusion_particle_count debe ser al menos {MIN_PARTICLE_COUNT} "
                f"para particle filter: {v}"
            )
        return v
    ...
    def validate_prediction_horizon(cls, v: float, info) -> float:
        ...
        if enable_path_prediction and v < MIN_PREDICTION_HORIZON:
            raise ValueError(
                f"prediction_horizon debe ser al menos {MIN_PREDICTION_HORIZON}s "
                f"si path_prediction está activo: {v}"
            )
        return v
    ...
    def validate_reid_cache(cls, v: int, info) -> int:
        ...
        if enable_reidentification and v < MIN_REID_CACHE_SIZE:
            raise ValueError(
                f"reid_cache_size debe ser al menos {MIN_REID_CACHE_SIZE} "
                f"si re-identificación está activa: {v}"
            )
        return v
...
class VisualizationConfig(BaseModel):
    ...
    show_track_ids: bool = Field(
        default=True, description="Mostrar IDs numéricos de los tracks en la visualización"
    )
    ...
    show_track_arrows: bool = Field(
        default=True, description="Mostrar flechas de dirección en los tracks"
    )
    show_track_speed: bool = Field(default=True, description="Mostrar velocidad en tiempo real")
    show_track_confidence: bool = Field(default=True, description="Mostrar confianza del track")
    track_circle_style: Literal["solid", "outline", "pulse"] = Field(
        "solid", description="Estilo de círculo para tracks: solid, outline, pulse"
    )
