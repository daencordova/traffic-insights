...
class FeatureManager(LoggerMixin):
    ...
    def __init__(
        ...
        self.feature_cache = FeatureCacheManager(
            max_size=max_cache_size, max_age_seconds=max_age_seconds
        )
    ...
    def extract_features(
        self, frame: np.ndarray, bbox: tuple, confidence: float = 0.5
    ) -> np.ndarray | None:
        ...
        try:
            features = self.feature_extractor.extract_features(frame, bbox, confidence)
            if features is not None:
                self._stats["features_extracted"] += 1
            return features
        except Exception as e:
            self.logger.debug("Error extrayendo features", error=str(e))
            return None

    def cache_features(self, track_id: int, features: np.ndarray, confidence: float = 0.5) -> None:
    ...
    def find_similar_tracks(
        ...
        candidates = self.feature_cache.find_candidates(
            features, position, max_candidates, self.similarity_threshold, self.spatial_threshold
        )

        candidates = [c for c in candidates if c.track_id not in exclude_tracks]
    ...
    def compare_features(self, features1: np.ndarray, features2: np.ndarray) -> float:
        ...
        try:
            ...
            self.logger.debug("Error comparando features", error=str(e))
            return 0.0
